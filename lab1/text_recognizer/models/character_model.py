...
class CharacterModel(Model):
    ...
    def predict_on_image(self, image: np.ndarray) -> Tuple[str, float]:
        ...
        input_img = np.expand_dims(image, 0) # make it (B, H, W, C)
        pred_digit = self.network.predict(input_img, batch_size=1).flatten()
        ind = np.argmax(pred_digit) # get predicted digit
        predicted_character = self.data.mapping[ind]
        confidence_of_prediction = pred_digit[ind]
